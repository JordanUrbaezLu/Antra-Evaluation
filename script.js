...
const Controller = ((model, view) => {
    ...
    const filterCompletedList = () => {
        const input = document.querySelector(view.domstr.filterinput);
        input.addEventListener('keyup', () => {
            const ul = document.getElementById("completed__todos");
            const li = ul.querySelectorAll("li.todo");
            for(let x = 0; x < li.length; x++) {
                let content = li[x].innerHTML;
                if (content.includes(input.value)) {
                    li[x].parentElement.style.display = "";
                } else {
                    li[x].parentElement.style.display = "none";
                }
            }
        });
    }
